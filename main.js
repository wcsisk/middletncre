/* ─────────────────────────────────────────────────────────
   ...
   Pulls live CSV data from two published Google Sheets:
     1. Lease Comps
     2. Sale Comps
   Renders sortable, filterable tables with pagination.
───────────────────────────────────────────────────────── */
...
function parseCSV(text) {
  ...
  let cur = '', inQ = false;
  const cells = [];
  const push = () => { cells.push(cur.trim()); cur = ''; };

  for (let i = 0; i < text.length; i++) {
    const c = text[i];
    if (c === '"') { inQ = !inQ; continue; }
    if (c === ',' && !inQ) { push(); continue; }
    if ((c === '\n' || c === '\r') && !inQ) {
      push();
      lines.push([...cells]);
      cells.length = 0;
  ...
  if (cur || cells.length) { push(); lines.push([...cells]); }
...
/* ── LEASE TABLE DEF ──
   Sheet columns: Tenant, Tenant Industry, Address, City, Zip, Bldg Name,
   Building Class, Sign Date, Commencement Date, Expiration Date, Submarket,
   Lease Size, Transaction Type, First Year Rent per area, Rent Type,
   Term (mos), TIs per SF, Escalation, Free Rent (mos)
*/
const LEASE_COLS = [
...
/* ── SALE TABLE DEF ──
   Sheet columns: Property Name, Property Address, City, Market, Total SF,
   Sale Price, $PSF, Cap Rate, Type of Sale, % Leased, Class Building,
   Seller, Buyer, Sale Date
*/
const SALE_COLS = [
...
function renderPagination(type, pages) {
  ...
  html += `<span class="page-info">${(cur-1)*PAGE_SIZE+1}–${Math.min(cur*PAGE_SIZE, state[type].filtered.length)} of ${state[type].filtered.length}</span>`;
...
async function loadAll() {
  ...
  if (results[0].status === 'fulfilled') {
    ...
    if (w) w.innerHTML = '<div class="comp-state error">Could not load lease comps — check your sheet is published</div>';
    console.error('Lease sheet error:', results[0].reason);
  }
  ...
  if (results[1].status === 'fulfilled') {
    ...
    if (w) w.innerHTML = '<div class="comp-state error">Could not load sale comps — check your sheet is published</div>';
    console.error('Sale sheet error:', results[1].reason);
  }
}
